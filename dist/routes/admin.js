...
router.get('/users', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
...
router.put('/users/:id/role', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
        const { role } = req.body;
        ...
        const user = await User.findByIdAndUpdate(userId, { role, updatedAt: new Date() }, { new: true });
        if (!user) {
...
// Get dashboard stats
router.get('/dashboard/stats', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
...
// Get audit logs
router.get('/audit-logs', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
...
router.put('/db/update/:collection/:id', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
...
router.delete('/db/delete/:collection/:id', authenticateToken, isMainAdminOrOwner, async (req, res) => {
    try {
        const { collection, id } = req.params;
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ error: 'Invalid document ID' });
